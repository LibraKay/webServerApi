  ...
  this.setStepsLeft = function(){
    ...
    for(var i = start; i< end; i++){
      ...
      html += '<div class="step" index="%s">'.format(step.index) +
                '<img src="/static/image/step_%s.svg" alt="%s.svg"/>'.format(step.status, step.status) +
                ...
                '<img class="step-context" src="/static/image/eye.svg" alt="eye.svg" index="%s"/>'.format(step.index) +
              '</div>'
    }
    ...
    try{
      return ("<div class='step-infos'>"+
                "<div class='infos-li'>" +
                  ...
                  "<img src='/static/image/step_%s.svg'>" +
                "</div>" +
                "<div class='infos-li'>" +
                ...
                "</div>" +
              "</div>").format(success, pass, success,
  ...
  this.getStepRightScrren = function(step){
    if(step.screen && step.screen.src){
      ...
      for(var i=0; i < step.screen.pos.length; i++){
        ...
        targets += "<img class='target' src='/static/image/target.png' rect=%s>"
                  .format(rect)
      }
...
function loadUrlInfo(){
  ...
  if(args.type) {
    ...
    back = '<a href="%s#detail" class="back" title="Back to multi-device report"><img src="/static/image/back.svg"></a>'.format(args.back)
...
function set_task_status(result){
  src = "/static/image/%s.svg".format(result=='Passed' ? 'success' : 'fail')
...
$(function(){
  ...
  $("img").error(function () {
    ...
    $(this).unbind("error")
    .addClass('error-img')
    .attr("src", "/static/image/broken.png")
